...
#define TAG "com.kangacoders.tikangavoip"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tikangavoip_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tikangavoip.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TikangavoipBindings::lookupGeneratedInit(
...
static void Tikangavoip_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tikangavoip_getBinding)->GetFunction());
}

static void Tikangavoip_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TikangavoipBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TikangavoipBinding = {
	"com.kangacoders.tikangavoip",
	Tikangavoip_init,
	Tikangavoip_dispose
};
...
Java_com_kangacoders_tikangavoip_TikangavoipBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.kangacoders.tikangavoip", &TikangavoipBinding);
	titanium::KrollBindings::addExternalLookup(&(::TikangavoipBindings::lookupGeneratedInit));
}
